...
void getselfmac(DWORD ip)//获得本地IP地址以及对应的MAC地址
{
	...
	//将ARPFrame.SendHa即发送方MAC设置为本机网卡的MAC地址
...
void routetable::add(routeitem* a)
{
	...
	else
	{
		...
		{
			if (a->mask < pointer->mask && a->mask >= pointer->nextitem->mask || pointer->nextitem == tail)//对掩码进行限制
				break;
		}
	...
	for (int i = 0; p != tail; p = p->nextitem, i++)//重新进行编号
	{
		p->index = i;
	}
	num++;
	...
	while (1)
	{
		...
		if (compare(header->DesMAC, selfmac))//目的mac是自己的mac
		{
			...
			else if (ntohs(header->FrameType) == 0x800)//IP格式数据报
			{
				...
				DWORD ip_ = rtable.lookup(ip1_);//查找是否有对应表项，ip_为下一跳地址
						...
						if (!t1 && !t2)
						{
							...
							else if (ip_ != -1)//非直接投递，查找下一条IP的MAC
							{
								if (!arptable::lookup(ip_, mac))
									arptable::insert(ip_, mac);//获取mac地址并且插入到arp表中
								//getmac(mac);
								resend(temp, mac);
							}
						}
